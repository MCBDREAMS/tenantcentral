...
function markdownToFragment(markdown) {
  return ReactDOMServer.renderToStaticMarkup(
    <ReactMarkdown
      components={{
        h1: ({ children }) => <h1 style={{ fontSize: 22, fontWeight: 700, color: "#0f172a", borderBottom: "1px solid #e2e8f0", paddingBottom: 6, marginBottom: 12, marginTop: 20 }}>{children}</h1>,
        h2: ({ children }) => <h2 style={{ fontSize: 18, fontWeight: 700, color: "#1e293b", marginTop: 26, marginBottom: 13 }}>{children}</h2>,
        h3: ({ children }) => <h3 style={{ fontSize: 15, fontWeight: 600, color: "#334155", marginTop: 18, marginBottom: 9 }}>{children}</h3>,
        h4: ({ children }) => <h4 style={{ fontSize: 13, fontWeight: 600, color: "#475569", marginTop: 14, marginBottom: 7 }}>{children}</h4>,
        p: ({ children }) => <p style={{ fontSize: 11, lineHeight: 1.75, color: "#334155", margin: "10px 0" }}>{children}</p>,
        li: ({ children }) => <li style={{ fontSize: 11, lineHeight: 1.75, color: "#334155", margin: "9px 0" }}>{children}</li>,
        ul: ({ children }) => <ul style={{ paddingLeft: 18, margin: "14px 0" }}>{children}</ul>,
        ol: ({ children }) => <ol style={{ paddingLeft: 18, margin: "14px 0" }}>{children}</ol>,
        ...
        pre: ({ children }) => <pre style={{ fontFamily: "Consolas, monospace", fontSize: 10, background: "#f8fafc", border: "1px solid #e2e8f0", padding: 10, borderRadius: 6, overflow: "auto", margin: "10px 0" }}>{children}</pre>,
        table: ({ children }) => <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 10, margin: "18px 0", border: "1px solid #cbd5e1" }}>{children}</table>,
        thead: ({ children }) => <thead style={{ background: "#f1f5f9" }}>{children}</thead>,
        th: ({ children }) => <th style={{ textAlign: "left", padding: "8px 10px", fontWeight: 700, color: "#334155", border: "1px solid #cbd5e1" }}>{children}</th>,
        td: ({ children }) => <td style={{ padding: "7px 10px", color: "#475569", border: "1px solid #e2e8f0" }}>{children}</td>,
        blockquote: ({ children }) => <blockquote style={{ borderLeft: "3px solid #3b82f6", paddingLeft: 12, color: "#475569", margin: "12px 0", fontStyle: "italic" }}>{children}</blockquote>,
        hr: () => <hr style={{ border: 0, borderTop: "1px solid #e2e8f0", margin: "18px 0" }} />,
      }}
    >
      {markdown}
    </ReactMarkdown>
